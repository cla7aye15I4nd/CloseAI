...
if args.load_model:
    ...
    memory = PrioritizedReplayMemory(50000, alpha)
    steps = 0
...
def optimize_model():
    ...
    elementwise_loss = compute_loss(transitions)
    ...
    memory.update(indices, new_priorities)

    return loss.item()

def main():
    global memory, policy_net, target_net, steps, env_id, beta_start
    ...
    while steps < args.frame_num:
        ...
        score = 0
        for t in count():
            beta = min(1.0, beta_start + steps * (1.0 - beta_start) / args.frame_num)
            env.render()
